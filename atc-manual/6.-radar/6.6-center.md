...
Aircraft at cruise may utilize the "Resume Flight" feature if all criteria has been met which means aircraft may unexpectedly return at cruise. Controllers are not responsible for a loss of separation for returning players, though efforts **should**{.red} be made to reestablish the appropriate separation minima as soon as practical.



**Resume Flight Criteria:**

- Altitude > 28,000ft MSL
- Level flight
- LNAV activated
- Estimated time en-route (ETE) to destination > 30 minutes
- On flight plan track

 

### 6.6.7

In the cruise phase speed control is essential in traffic flow management, controllers **must**{.red} ensure that they are familiar with typical Mach cruising speeds for aircraft under their control *(see [6.15.5](/guide/atc-manual/6.-radar/6.15-aircraft-speed#6.15.5) below)*. An increase or decrease of Mach .01 can change an aircraft's arrival time significantly (dependent on how far ahead the instruction is sent). Therefore to control traffic flow, Center Controllers **should**{.red} utilize speed control where possible (although altitude changes and vectors are always available if required). This will reduce Controller workload and prevent aircraft being taken hundreds of miles off their flight plan.
