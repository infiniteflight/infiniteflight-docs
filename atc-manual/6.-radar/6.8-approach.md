...
- ensure that all aircraft involved are **LEVEL** at the altitude you intend on clearing them at for the approach, all separated by 1000ft as a minimum (ideally before the base turn)
- vector all aircraft so that the intercepts are abeam each other (this will mean that no aircraft has begun to descend on the G/S, which could compromise vertical separation before aircraft intercepting are established)
